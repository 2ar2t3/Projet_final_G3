...
class Turbulence():
    ...
    def search_turbulences(self, states):
        ...
        plane_counts = states['nom'].value_counts()

        # Garde que les avions qu'on voit constamment
        valid_noms = plane_counts[plane_counts >= 2].index

        # Filtre states
        states = states[states['nom'].isin(valid_noms)]

        #//////////////////////////////////////////////////////////////////////////////////////////////////////////////
        print(f"Number of unique planes: {states['nom'].nunique()}")
        ...
        # /////////////////////////////////////////////////////////////////////////////////////////////////////////////
        print(f"Nombre d'avions dans alt_stats = {len(alt_stats)}")
        ...
        outliers = states[
            (states['altitude'] > states['alt_mean'] + 3*states['alt_std']) |
            (states['altitude'] < states['alt_mean'] - 3*states['alt_std'])
            ]

        # Extrait la longitude et latitude en NumPy array
        resultat = outliers[['longitude', 'latitude', 'altitude']].to_numpy()
        # ------------------------ Remplacer avec:  ------------------------------------------------------------------

        # resultat = self.group_turbulence(states) # fonction a creer en dessous
        ...
        print(resultat[0:3])
        print(f"length of turbulences: {len(resultat)}")

        return resultat


    # def group_turbulence(self):
    #
    #     3 arrays pour les etapes
    #
    #     1. resultat de maintenant
    #     2. temporaire (avion dedans)
    #     3. l'avion est sortie de la turbulence et on l'envoie
